...
/**
 * Generates transcript from audio - using Gemini to "reconstruct" a transcript
...
请生成这份带有精准时点的详尽逐字稿：`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          temperature: 0.7,
          maxOutputTokens: 8192, // Gemini supports up to 8192
          topP: 0.8,
          topK: 40
        }
      }),
...
/**
 * Summarizes podcast intro text to 2-3 sentences using Gemini
 */
...
/**
 * Transcribes voice input - placeholder for Gemini
...
export async function transcribeVoice(_audioBlob: Blob, _timeout: number = 10000): Promise<string> {
  // Gemini doesn't have direct voice transcription API for browser blobs
  // Use Web Speech API as fallback
  return new Promise((resolve, reject) => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      reject(new Error('Voice transcription is not supported in this browser. Gemini does not provide audio transcription API.'));
      return;
    }
